...
public:
  ...
  [[nodiscard]] std::string_view name() const { return info_->layerName ? info_->layerName : ""; }

  [[nodiscard]] NvDsInferDataType data_type() const { return info_->dataType; }

  [[nodiscard]] std::uint32_t num_dims() const { return info_->inferDims.numDims; }

  [[nodiscard]] std::uint32_t num_elements() const { return info_->inferDims.numElements; }

  // Raw host-side output buffer (cast to the appropriate type based on data_type()).
  [[nodiscard]] void* buffer() const { return info_->buffer; }
  ...
  [[nodiscard]] const NvDsInferLayerInfo* get() const { return info_; }
...
public:
  ...
  struct LayerRange {
    ...
    struct Iterator {
      ...
      TensorLayerView operator*() const { return TensorLayerView{ptr}; }
      Iterator& operator++() {
        ++ptr;
        return *this;
      }
    ...
    [[nodiscard]] Iterator end() const { return {data + count}; }
  ...
  [[nodiscard]] NvDsInferTensorMeta* get() const { return meta_; }
