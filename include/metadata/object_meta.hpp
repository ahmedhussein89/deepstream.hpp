...
struct BoundingBox {
  ...
  [[nodiscard]] float bottom() const { return top + height; }
...
public:
  ...
  [[nodiscard]] std::int32_t class_id() const { return meta_->class_id; }
  [[nodiscard]] std::uint64_t object_id() const { return meta_->object_id; }
  [[nodiscard]] float confidence() const { return meta_->confidence; }
  [[nodiscard]] float tracker_confidence() const { return meta_->tracker_confidence; }
  [[nodiscard]] std::string_view label() const { return meta_->obj_label; }
  [[nodiscard]] std::int32_t unique_component_id() const { return meta_->unique_component_id; }
  ...
  [[nodiscard]] NvDsObjectMeta* get() const { return meta_; }
