...
struct SmartRecord {
  ...
  static void start(GstElement* urisrcbin,
    ...
    g_signal_emit_by_name(G_OBJECT(urisrcbin),
                          "start-sr",
                          static_cast<guint>(source_id),
                          static_cast<guint>(session_id),
                          dir_c,
                          prefix_c,
                          static_cast<guint>(timeout_sec),
                          nullptr,
                          &ret);
  }
  ...
  static void stop(GstElement* urisrcbin, std::uint32_t source_id, std::uint32_t session_id) {
    ...
    g_signal_emit_by_name(
        G_OBJECT(urisrcbin), "stop-sr", static_cast<guint>(source_id), static_cast<guint>(session_id), nullptr, &ret);
  }
