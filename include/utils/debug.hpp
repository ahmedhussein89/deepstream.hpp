...
struct DebugMessage {
  ...
  [[nodiscard]] std::string_view level_str() const noexcept { return debug_level_str(level); }
  [[nodiscard]] std::string_view kind_str() const noexcept { return error_kind_str(kind); }
};
...
public:
  ...
  void set_min_level(DebugLevel level) noexcept { min_level_ = level; }

  [[nodiscard]] DebugLevel min_level() const noexcept { return min_level_; }
  ...
  void log(DebugLevel level, ErrorKind kind, std::string message, std::string_view file, int line) {
    ...
    if(logger_) {
      ...
      switch(level) {
        case DebugLevel::Debug: logger_->debug("{}", text); break;
        case DebugLevel::Info: logger_->info("{}", text); break;
        case DebugLevel::Warn: logger_->warn("{}", text); break;
        case DebugLevel::Error: logger_->error("{}", text); break;
        case DebugLevel::Off: break;
      }
    }
  }
